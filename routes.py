...
def register_course(id):
    ...
    if form.validate_on_submit():
        ...
        if stripe and current_app.config.get("STRIPE_SECRET_KEY"):
            ...
            db.session.commit()
            enrollment, user, user_created = _finalize_enrollment(registration)
            if user_created:
                login_user(user)
            return redirect(
                url_for("main_bp.course_access", enrollment_id=enrollment.id)
            )

    return render_template(
        "course_register.html", course=course, form=form, settings=settings
    )
...
def buy_course(id):
    ...
    enrollment = CourseEnrollment.query.filter_by(
        course_id=id, user_id=current_user.id
    ).first()
    ...
    if request.method == "POST":
        ...
        session = gateway.create_checkout_session(
            amount=course.price,
            currency="brl",
            success_url=url_for("main_bp.payment_success", _external=True)
            + "?session_id={CHECKOUT_SESSION_ID}",
...
def registration_success(registration_id):
    ...
    if registration.payment_status != "paid" and stripe and registration.payments:
        ...
        session = stripe.checkout.Session.retrieve(
            registration.payments[0].transaction_id
        )
    ...
    if registration.payment_status == "paid":
        ...
        if not user:
            username = registration.participant_email.split("@")[0]
            user = User(
                username=username, email=registration.participant_email, role="student"
            )
            user.set_password(secrets.token_urlsafe(8))
            db.session.add(user)
            db.session.flush()
        ...
        enrollment = CourseEnrollment.query.filter_by(
            course_id=registration.course_id, user_id=user.id
        ).first()
        if not enrollment:
            enrollment = CourseEnrollment(
        ...
        existing_txn = PaymentTransaction.query.filter_by(
            enrollment_id=enrollment.id, provider_id=provider_id
        ).first()
        if not existing_txn:
...
def hotmart_webhook():
    ...
    transaction = PaymentTransaction(
        enrollment_id=enrollment.id, amount=amount, provider_id=txn_id, status="paid"
    )
    db.session.add(transaction)
